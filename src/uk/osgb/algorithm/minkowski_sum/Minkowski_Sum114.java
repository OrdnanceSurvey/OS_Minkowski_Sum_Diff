/* this is an experimental implementation of Minkowski sum and difference based on JTS geometric functionality (version 1.14)
 * 
 ...
 * 
 * You will need JTS to use this library.
 *
 * version 0.1
 * 
 * date: 2018-02-14
...
public class Minkowski_Sum114 {
	...
	public static Geometry minkSum(Geometry src, Geometry ref, Boolean doNeg, Boolean isRefConvex) {
		try {
			...
			Class clz = Class.forName("uk.osgb.algorithm.minkowski_sum.Minkowski_Sum114"); 
	...
	/*
	public static Geometry compMinkSumGeometryCollectionPlg(GeometryCollection src, LineString ref, boolean doNeg, boolean isRefConvex) {
		Geometry rlt = null;
		int numParts = src.getNumGeometries();
		for(int i = 0; i < numParts; ++i) {
	...
	private static Geometry compMinkSumLS(LineString src, Coordinate[] refCoords, boolean isRefConvex) {
		...
		if(refCoords[0].equals2D(refCoords[refCoords.length-1])) { // reference is a linear ring
			sum = coordArrayVectorAddition(coords, refCoords, isRefConvex);
		}else {
			sum = coordArrayVectorAddition(coords, refCoords);
		}
		return sum;
	}
	//
	...
	private static Geometry compMinkSumPlg_backup(Polygon src, Coordinate[] refCoords, boolean isRefConvex) {
		...
		// expanding the shell of original geometry (on two directions)
		if(isRefRing) {
			sum = coordArrayVectorAddition(extCoords, refCoords, isRefConvex);
		}else {
			sum = coordArrayVectorAddition(extCoords, refCoords);
		}
		...
		if(numHoles>0) {
			for(int k = 0; k < numHoles; ++k) {
				...
				if(isRefRing) {
					sumHole = (Polygon) coordArrayVectorAddition(holeCoords, refCoords, isRefConvex);
				}else {
					sumHole = (Polygon) coordArrayVectorAddition(holeCoords, refCoords);
				}
				if(sumHole!=null) {
	...
	private static Geometry compMinkDiffPlg_backup(Polygon src, Coordinate[] refCoords, boolean isRefConvex) {
		...
		if(numHoles>0) {
			for(int k = 0; k < numHoles; ++k) {
				...
				Polygon sumHole = null;
				if(isRefRing) {
					sumHole = (Polygon) coordArrayVectorAddition(holeCoords, refCoords, true);
				}else {
					sumHole = (Polygon) coordArrayVectorAddition(holeCoords, refCoords);
				}
				if(sumHole!=null) {
	...
	// buffering holes in source geometry
		...
		if(numHoles>0) {
			for(int k = 0; k < numHoles; ++k) {
				...
				if(isRefRing) {
					diffHole = (Polygon) coordArrayVectorAddition(holeCoords, refCoords, true);
				}else {
					diffHole = (Polygon) coordArrayVectorAddition(holeCoords, refCoords);
				}
	...
	public static int getHoles(Polygon plg, Collection<LinearRing> holeCol) {
		...
		if(numHoles > 0) {
			for(int i = 0; i < numHoles; ++i) {
				LinearRing ring = gf.createLinearRing(plg.getInteriorRingN(i).getCoordinates());
				holeCol.add(ring);
			}
		}
		return numHoles;
	...
	public static LinearRing getShellRing(Polygon plg) {
		return gf.createLinearRing(plg.getExteriorRing().getCoordinates());
	}
	//
	/**
	 * @param plg
	...
	public static LinearRing[] getHoles(Polygon plg) {
		...
		if(numHoles>0) {
			...
			for(int i = 0; i < numHoles; ++i) {
				holes[i] = gf.createLinearRing(plg.getInteriorRingN(i).getCoordinates());
			}
			return holes;
		}else {
			return null;
	...
	public static LinearRing[] getShellRings(MultiPolygon mulPlg) {
		...
		if(numPlg>0) {
			...
			for(int i = 0; i < numPlg; ++i) {
				Polygon plg = (Polygon) mulPlg.getGeometryN(i);
				shells[i] = gf.createLinearRing(plg.getExteriorRing().getCoordinates());
			}
			return shells;
		}else {
			return null;
	...
	public static LineString negationLS(LineString ls){
		...
		return gf.createLineString(coordsNeg);
	}
	 ...
	 * @return
	 */
	public static Geometry segPlgAddition(Coordinate[] refCoords, Coordinate segSp, Coordinate segEp, boolean isRefConvex) {
		if(isRefConvex) {
	 ...
	 * @return
	 */
	public static Geometry segLSAddition(Coordinate[] refCoords, Coordinate segSp, Coordinate segEp) {
		int numPts = refCoords.length;
		/* checked outside
		if(refCoords[0].equals2D(refCoords[numPts-1])) { // linear ring
			return segPlgAddition(refCoords, segSp, segEp, false);
		}
		*/
		// otherwise, reference is an open linestring
		Geometry sum = null;
		for(int i = 0; i < numPts-1; ++i) {
			Coordinate refSp = refCoords[i];
			Coordinate refEp = refCoords[i+1];
			Geometry sum1 = segVectorAddition(segSp, segEp, refSp, refEp);
			if(sum == null) {
				sum = sum1;
			}else {
				if(sum1!=null) {
	...
	private static Geometry segVectorAddition(Coordinate segSp, Coordinate segEp, Coordinate refSp, Coordinate refEp){
		...
		if(hull.getDimension() > 1) {
			...
			return gf.createPolygon(null, null); // return an empty polygon?
		}
	}
	...
	public static Geometry coordArrayVectorAddition(Coordinate[] geomCoords, Coordinate[] refCoords) {
		int numPts = refCoords.length;
		if(refCoords[0].equals2D(refCoords[numPts-1])) { // linear ring
			return coordArrayVectorAddition(geomCoords, refCoords, false);
		}
		Geometry sumAll = null;
		for(int j = 0; j < geomCoords.length-1; ++j) {
			Coordinate segSp = geomCoords[j];
			Coordinate segEp = geomCoords[j+1];
			if(!segSp.equals2D(segEp)) {
				Geometry sum1 = segLSAddition(refCoords, segSp, segEp);
				if(sum1!=null) {
					if(sumAll == null) {
						sumAll = sum1;
					}else {
